...
<div class="content">
    ...
    <?php if (!empty($errors['geral'])): ?>
        <div class="flash-message flash-error">
            <span><?= sanitize($errors['geral']) ?></span>
        </div>
    <?php endif; ?>
    ...
    <form action="?page=auth/cadastro" method="POST">
        ...
        <div class="form-group">
            ...
            <input type="text" id="nome" name="nome" class="form-input" placeholder="Ex: João da Silva" value="<?= sanitize($data['nome'] ?? '') ?>" required>
            <?php if (!empty($errors['nome'])): ?>
                <div class="input-error-msg"><?= sanitize($errors['nome']) ?></div>
            <?php endif; ?>
        </div>

        <div class="form-group">
            <label class="form-label" for="cpf">CPF</label>
            <input type="text" id="cpf" name="cpf" class="form-input" data-mask="cpf" placeholder="000.000.000-00" value="<?= sanitize($data['cpf'] ?? '') ?>" required>
            <?php if (!empty($errors['cpf'])): ?>
                <div class="input-error-msg"><?= sanitize($errors['cpf']) ?></div>
            <?php endif; ?>
        </div>

        <div class="form-group">
            <label class="form-label" for="telefone">Telefone / WhatsApp</label>
            <input type="text" id="telefone" name="telefone" class="form-input" data-mask="telefone" placeholder="(83) 90000-0000" value="<?= sanitize($data['telefone'] ?? '') ?>" required>
        </div>

        <div class="form-group">
            <label class="form-label" for="email">E-mail</label>
            <input type="email" id="email" name="email" class="form-input" placeholder="seuemail@dominio.com" value="<?= sanitize($data['email'] ?? '') ?>" required>
            <?php if (!empty($errors['email'])): ?>
                <div class="input-error-msg"><?= sanitize($errors['email']) ?></div>
            <?php endif; ?>
        </div>
        ...
        <div class="form-group">
            ...
            <?php if (!empty($errors['senha'])): ?>
                <div class="input-error-msg"><?= sanitize($errors['senha']) ?></div>
            <?php endif; ?>
        </div>
        ...
        <div class="form-group">
            ...
            <?php if (!empty($errors['confirmar_senha'])): ?>
                <div class="input-error-msg"><?= sanitize($errors['confirmar_senha']) ?></div>
            <?php endif; ?>
        </div>
        ...
        <?php if (!empty($errors['lgpd'])): ?>
            <div class="input-error-msg" style="margin-top: -10px; margin-bottom: 16px;"><?= sanitize($errors['lgpd']) ?></div>
        <?php endif; ?>
